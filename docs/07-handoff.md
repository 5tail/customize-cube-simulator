...
**下一步**：
1. 使用者 merge 打亂/復原 PR → Pages 驗收。
2. **新 Phase 2（最後的功能）**：(a) 一鍵截圖：按鈕把目前 3D 畫面存成圖片下載（注意 WebGL 截圖需在 render 後立即讀，或開 preserveDrawingBuffer）。(b) 費用試算：計價參數放 repo 設定檔（如 src/pricing.json），店家 GitHub 網頁改參數 → 自動重部署生效。**計價公式待使用者提供**（例：基本價？印幾面加多少？數量折扣？）——動工前先問。
3. **Phase 1 總驗收**：手機操作一輪（視角、選圖、四滑桿、打亂復原）。
