...
**下一步**：
1. 使用者 merge 本 PR → Pages 驗收（撥層手感、視角、打亂復原迴歸）。
2. **全案總驗收**：手機操作一輪（視角、撥層、選圖、四滑桿、打亂復原、試算、截圖）。全過＝**目前規格的功能全部完工**。
3. 之後只剩維護與微調；若日後訂單量大要重啟站內送件（Supabase/Resend），規格見 docs/01「已暫緩」區。撥層手感若使用者覺得太靈敏/太鈍，調 RubiksCube.jsx 的 DRAG_SENSITIVITY 一個數字即可。
