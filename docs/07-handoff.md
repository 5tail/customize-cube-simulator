...
**下一步**：
1. 使用者 merge 旋轉 PR → Pages 驗收。
2. **打亂/復原按鈕**（docs/01 已列，使用者選了按鈕方案、不做手動撥層）：單層旋轉引擎（27 顆位置＋朝向的資料層，可測）＋動畫，打亂 15–20 步、復原＝回整齊。
3. **Phase 1 總驗收**：手機實際操作一輪（旋轉視角、選圖、四滑桿、打亂復原）。過了 Phase 1 完工，進 Phase 2（需使用者開 Supabase/Resend 帳號）。
